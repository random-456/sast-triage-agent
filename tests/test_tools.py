...
class TestFileOperations:
    ...
    def test_read_file_normal(self, test_codebase, monkeypatch):
        ...
        monkeypatch.setattr("sast_triage.agent_tools.CODEBASE_DIR", test_codebase)
...
class TestFindingOperations:
    ...
    def test_get_finding_details(self, test_findings_path):
        ...
        details = get_finding_details.invoke(
            {"result_hash": "hash-001", "json_path": json_path}
        )
        ...
        details = get_finding_details.invoke(
            {"result_hash": "nonexistent", "json_path": json_path}
        )
...
class TestSearchInFiles:
    ...
    def test_search_single_extension(self, test_codebase, monkeypatch):
        """Test searching in files with a single extension."""
        monkeypatch.setattr("sast_triage.agent_tools.CODEBASE_DIR", test_codebase)
        ...
        result = search_in_files.invoke(
            {"pattern": "NONEXISTENT_PATTERN_XYZ", "file_extensions": "js"}
        )
        assert "error" not in result
        assert result["matches_found"] == 0

    def test_search_multiple_extensions(self, test_codebase, monkeypatch):
        ...
        assert has_js, "Expected at least one .js match"
        assert has_py, "Expected at least one .py match"

    def test_search_all_files(self, test_codebase, monkeypatch):
    ...
    def test_search_default_all_files(self, test_codebase, monkeypatch):
    ...
    def test_search_results_limit(self, test_codebase, monkeypatch):
        """Verify results are capped at MAX_SEARCH_RESULTS."""
        monkeypatch.setattr("sast_triage.agent_tools.CODEBASE_DIR", test_codebase)
        monkeypatch.setattr("sast_triage.agent_tools.MAX_SEARCH_RESULTS", 1)
