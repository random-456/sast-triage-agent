...
class FindingsHelpers:
    ...
    @classmethod
    def save_findings_data(cls, triage_records: List[dict], detailed_records: List[dict]) -> None:
        """Save findings data to CSV and JSON files."""

        os.makedirs(FINDINGS_DIR, exist_ok=True)

        # Write CSV file
        cls.logger.info(f"Saving {len(triage_records)} records to {FINDINGS_CSV_FILE}...")
        ...
        # Write JSON file
        cls.logger.info(f"Saving detailed records to {FINDINGS_JSON_FILE}...")

        with open(FINDINGS_JSON_FILE, "w", encoding="utf-8") as f:
            json.dump(detailed_records, f, indent=4)

        cls.logger.info("Findings data saved successfully")